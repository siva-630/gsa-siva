const prompts = [
    {
        id: 'prompt-1',
        title: 'brainstorm',
        description: 'Ask me 5 personality MCQs; match me to a famous artist/designer.',
    ...
    },
    {
        id: 'prompt-2',
        title: 'plan',
        description: 'Plan a small, festive get-together for 5 friends at home.',
    ...
    },
    {
        id: 'prompt-3',
        title: 'historical',
        description: 'Pick a historical movie and bust 3 fun inaccuracies.',
    ...
    },
    {
        id: 'prompt-4',
        title: 'weekend plan',
        description: 'Suggest a location for a friends weekend getaway.',
    ...
    },
    {
        ...
        description: '5 eco-friendly festive decor ideas for home/hostel.',
    ...
    grid.innerHTML = prompts.map((p, i) => {
        ...
        return `
            <article class="card ${borderClass}" id="card-${p.id}">
                ...
                <div class="card-actions sp">
                  <button class="sparkle-button" data-action="open" data-id="${p.id}">
                    <span class="spark"></span>
                    <span class="backdrop"></span>
                    <svg class="sparkle" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function copyLinkForPrompt(id) {
    const p = getPromptById(id);
    const textToCopy = (p && p.externalLink)
        ? p.externalLink
        : (() => { const url = new URL(window.location.href); url.hash = id; return url.toString(); })();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    delegate(grid, 'button[data-action="open"]', 'click', (_, btn) => {
        ...
        if (prompt.externalLink) {
            window.open(prompt.externalLink, '_blank', 'noopener');
        } else {
            openModalWithPrompt(prompt);
        }
    });
    delegate(grid, 'button[data-action="copy"]', 'click', (_, btn) => {
        const id = btn.getAttribute('data-id');
        copyLinkForPrompt(id);
    });
