    ...
    btn.addEventListener('click', () => {
        ...
        promptsn.forEach((link, index) => {
            setTimeout(() => {
                ...
                if (!newTab) blockedCount++;
            }, index * 1000); // small delay for smooth opening
        });

        // Remove notification after clicking
        notification.remove();

        // Show message if tabs are blocked
        setTimeout(() => {
            if (blockedCount > 0) {
                alert(
                    `⚠️ Some prompts were blocked by your browser. ` +
                    `Please allow pop-ups to run all prompts at once.`
                );
            }
        }, promptsn.length * 1200);
    });
